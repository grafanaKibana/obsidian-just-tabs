...
**Defaults and global controls** is split into **General**, **Tab appearance**, **Layout**, **Icons and labels**, and **Nested blocks** groups. It contains Size, Personality, Overflow behavior, Palette, Accent, Alignment, theme button outline, underline thickness and placement, tab gap and radius, horizontal padding, side-list width, icon size and spacing, selected-tab weight, content spacing, and nested Card/Flat styling. Separator uses short, centered divider elements only between controls on the same visual row or column. Rail uses a padded rounded track with compact tab padding and a selected segment. Selected labels use the Default 600 weight unless Thinner (400) or Bolder (700) is chosen. Underline Auto uses bottom for Top/Bottom and mounted controls, right for Left, and left for Right; explicit Top/Right/Bottom/Left placement overrides it. Nested controls always use a quiet theme-derived tint so each level stays visually distinct. Padding and side-list width sliders apply immediately; Left/Right tabs always share equal width, including their narrow full-width row fallback. Equal width shares unused space but keeps each label intact, so excess Top/Bottom tabs scroll or wrap instead of squeezing their text.
